...
module Lib(Tree(..), Color(..), empty, contains, put, delete) where
...
contains x E    = False
contains x (T _ a y b)
  | x < y     = contains x a
  | x == y    = True
  | otherwise = contains x b

put :: (Ord a) => a -> Tree a -> Tree a
put x s = makeBlack $ ins s
...
instance Functor Tree where
    fmap _ E = E
    fmap f (T color a y b) = (T color (fmap f a) (f y) (fmap f b))

instance Arbitrary a => Arbitrary (Tree a) where
    arbitrary = sized $ genTree R

genTree :: (Arbitrary a2, Integral a1) => Color -> a1 -> Gen (Tree a2)
genTree B 0 = return E
genTree B 1 = 
  oneof [return E,
...
genTree B n | n>0 = 
  ...
  where subtree  = genTree B (n `div` 2)
        subtree' = genTree R n

genTree R 0 = return E
genTree R 1 = return E
genTree R n | n>0 = 
  oneof [liftM4 T (return B) subtree arbitrary subtree]
  where subtree = genTree B (n `div` 2)